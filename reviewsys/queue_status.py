...
def queue_body(
    ...
    *,
    ...
    if priority:
        head = f"⚡ {tag}Priority review — {_ordinal(position)} in line, starts as soon as a slot frees"
    else:
        head = f"🕓 {tag}Queued for automated review — {_ordinal(position)} in line, estimated start in {_fmt_minutes(eta_minutes)}"
    lines = [
        github.GATE_MARKER,
        f"{head} (commit {sha[:8]})",
