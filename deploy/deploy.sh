...
TAG="${1:?usage: deploy.sh <tag> [--force]}"
FORCE="${2:-}"
...
ssh -o BatchMode=yes "$HOST" bash -s -- "$TAG" "$SHA" "$LABEL" <<'REMOTE'
set -euo pipefail
TAG="$1"; SHA="$2"; LABEL="$3"
...
cp "$SRC/deploy/ai.thepastaclaw.reviewsys.plist" "$HOME/Library/LaunchAgents/$LABEL.plist"
...
if ! launchctl list | grep -q "$LABEL"; then launchctl load -w "$HOME/Library/LaunchAgents/$LABEL.plist" 2>/dev/null || true; fi
sleep 3
